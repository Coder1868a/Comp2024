...
void pre_auton(void) {
  ...
  intake.setVelocity(150, rpm); //Sets rpm of intake
  LeftDT.setVelocity(270,rpm);
  RightDT.setVelocity(270,rpm);
  ramp.setVelocity(300, rpm);
...
int inertialTurning(int desireddegrees){
  LeftDT.setVelocity(100, rpm);
  RightDT.setVelocity(100,rpm);
  inertialSensor.setHeading(desireddegrees, degrees);
  while(inertialSensor.heading(degrees) > inertialSensor.heading(degrees) - desireddegrees){
    LeftDT.spin(forward);
    RightDT.spin(reverse);                                                                                    
    wait(20, msec);
  }
  return 1;
}
...
void autonomous(void) {
  ...
  // couldn't run the auton task so I just let the motors spin forward and it worked earlier
  // but I had to reverse something and it doesn't work anymore.  
  ...
  LeftDT.spinFor(reverse, inchestodegrees(15), degrees, false);
  RightDT.spinFor(reverse, inchestodegrees(15), degrees, false);
  ...
  // Next task after backwards movement is calculated.

  // vex::task::sleep(750);

  // turn code

  // ramp.spin(forward, 1, voltageUnits::volt);
...
void usercontrol(void) {
  ...
  isPIDon = false;
  while (1) {
    int leftmotorspeed = 1.9*(expoTurning(Controller1.Axis3.position())) + (expoTurning(Controller1.Axis1.position()));
    int rightmotorspeed = 1.9*(expoTurning(Controller1.Axis3.position())) - (expoTurning(Controller1.Axis1.position()));
